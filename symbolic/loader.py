...
from .symbolic_types import SymbolicInteger, getSymbolic, SymbolicStr,SymbolicDict, SymbolicBytes
...
class Loader:
	...
	def createInvocation(self):
		...
		#Get entry point function arguments
		...
		if "symbolic_args" in func.__dict__:
			for (f,v) in func.symbolic_args.items():
				if not f in argspec.args:
					...
					# v is the concrete value
					# s is the symoblic_type, e.g., SymbolicStr
					s = getSymbolic(v)
					if (s == None):
		...
		for a in argspec.args:
			if not a in inv.getNames():
				#By default, treat as Integer and start with 0
	...
	# f is the name, val is the value, st is the symbolic type and inv is the function invocation
	def _initializeArgumentSymbolic(inv,f,val,st):
		if st.__dict__['__module__'].find('dict') !=-1:
