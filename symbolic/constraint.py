...
class Constraint:
	...
	def __init__(self, parent, last_predicate):
		self.inputs = None # the inputs lead to this constraint
		...
		#Bind the path and the execution result of the selected input on that path
	...
	def removeDirectPartent(self):
		tmp = self.parent
		if tmp.predicate is not None:
			tmp.hidden = True
			tmp = tmp.parent
			self.parent = tmp
			tmp.children.append(self)
		return

	def getDirectAssertAndQuery(self):
		# collect the assertions
		asserts = None
		tmp = self.parent
		while tmp.predicate is not None:
			asserts = tmp.predicate
			break
