...
class CVCString(CVCExpression):
    ...
    def constant(cls, v, solver):
        ...
        try:
            ...
            # The AssertationError is caused by the check of whether  a SymbolicStr startswith('\ufeff'). 
            # JSON loads will make the verification
            # in this case, the assertion fails. 
            #Regardless, lets still return the unequal cvcstr with the aim to explore one more path.
    ...
    def regcontains(self, regexp):
        regexp = self.em.mkExpr(CVC4.STRING_TO_REGEXP, self.em.mkConst(CVC4.CVC4String(str(regexp))))
        return CVCExpression(self.em.mkExpr(CVC4.STRING_IN_REGEXP,
            self.cvc_expr, regexp), self.solver)
