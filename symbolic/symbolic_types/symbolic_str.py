...
class SymbolicStr(SymbolicObject, str):
    ...
    def startswith(self, prefix):
        # We do not consider ufeff, bom.
        # For this case, we do not add it into the expression tree.
        if prefix== '\ufeff':
            return self.val.startswith(prefix)
    ...
    #In python3, the encoding will convert string to bytes    
    def encode(self, encoding="utf-8", errors="strict"):
        encoded_str = self.val.encode(encoding)
        if encoded_str.startswith(codecs.BOM_UTF8):
            pass
           
        if isinstance(encoded_str, bytes):
            from symbolic.symbolic_types.symbolic_bytes import SymbolicBytes 
            return SymbolicBytes(self.name, encoded_str, self.expr)
