...
class CVCWrapper(object):
    options = {'produce-models': 'true',
               ...
               'rewrite-divk': 'true',
               # Per Query timeout of 50 seconds
               'tlimit-per': 120000,
    ...
    def findCVCExpr(self, asserts, query):
        # return a simplified cvcexpr
        exprbuilder = ExprBuilder(asserts, query, self.solver)
        cvc_expr = exprbuilder.query.cvc_expr
        returnStr = self.solver.simplify(cvc_expr).toString()
        if '"code" code' in returnStr:
            import pdb; pdb.set_trace()
        returnStr = re.sub(r'\s\|"', ' "', returnStr)
        returnStr = re.sub(r'"\|\s', '" ', returnStr)
        returnStr = re.sub(r'"\|\)', '")', returnStr)
        return returnStr
