...
from symbolic_types.symbolic_bytes import SymbolicBytes
...
def get_user_from_cookie(cookies, app_id, app_secret):
    ...
    """
    # For dict, we need to add the expr for ourself use.
    #import pdb; pdb.set_trace()

    cookie = cookies.getSym("fbsr_" + app_id, "")
    print(type(cookie))
    if not cookie:
        return None
    parsed_request = parse_signed_request(cookie, app_secret)
    #print("parsed_request is:")
    ...
    try:
        ...
        if len(parsed_request["code"]) !=0:
            return {'id':parsed_request['user_id'],'name':"alice",'link':'http://www.example.com','access_token':"EAABzj3PSNiUBAEBc5Y29IQoOgm0cIFXASNZAbmBRH4w08ZCFnzPVUvCMK56LGGr450ZA8YuXfJydqd70KZATBUH7RWDSkGtpjkX0jApDyCx87oP6sZCgGrp02gSFp7uKZBZBGoqKjBSYmo1PQRpqrwcL3jzZBucndurfqKakdmBXzXZBv96209o1sQqO8iBtlWuS93cA1mP1eLwZDZD"}
...
def parse_signed_request(signed_request, app_secret):
    ...
    """
    try:
        encoded_sig = signed_request['sig']
        payload = signed_request['value']

        data = base64.urlsafe_b64encode(str(payload).encode('ascii'))
        #encoded_sig, payload = signed_request.split('.', 1)
        #print(encoded_sig.expr)
        # here sig_len is the type of SymbolicInteger. so we need to cast it to int type
        #sig_len = ((4 - len(encoded_sig) % 4) % 4)

        # It converts the encoded_sig to bytes. Currently, we do not add the path constraint on this operation.
        #sig = base64.urlsafe_b64decode(encoded_sig + "=" * int(sig_len))
                                       # ((4 - int(len(encoded_sig) % 4) % 4)))
        #print(type(sig))
        #sig = SymbolicBytes("se", sig)
        #print("the expr after base64.urlsafe_b64decode")
        #print(sig.expr)
        #print("hh: "+ sig)
        #data = base64.urlsafe_b64decode(payload + "=" *
        #                               int(((4 - len(payload) % 4) % 4)))
    except IndexError:
       ...
       #signed request.split only return one value
        return False
    #decoded_data = data.decode('ascii')
    '''
     need to modify json.loads so that data is also Symbolicized
      Meanwhile, we need to remember the operation so that we can map data to the original input
    '''
    # try:
    #     data = json.loads(decoded_data)
    # except Exception:
    #     print("error when perfom json.loads")
    #     return False

    algorithm = payload.getSym('algorithm', '')
    ...
    encoded_payload = str(payload).encode('ascii')
...
if __name__=='__ main__':
    FB_APP_ID = '898217060231635'
    FB_APP_NAME = 'LoginSample'
    FB_APP_SECRET = '96be33e88cc1b2f65e7ffbd7b72d77cd'
