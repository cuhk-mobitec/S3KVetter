...
def parse_authorization_code_response(uri, state=None):
    ...
    **state**
    ...
    if not 'code' in params:
        ...
        print("Missing code parameter in response.")
...
def my_parse_authorization_code_response(uri, params, state=None):
    if not is_secure_transport(uri):
        #raise InsecureTransportError()
        print("no tls is inseucre")
...
def validate_token_parameters(params):
    ...
    if params.scope_changed:
        message = 'Scope has changed from "{old}" to "{new}".'.format(old=params.old_scope, 
            new=params.scope)
