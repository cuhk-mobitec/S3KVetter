...
def prepare_grant_uri(uri, client_id, response_type, redirect_uri=None,
    ...
    if not is_secure_transport(uri):
        #print("in secure transport is not allowed")
        #return None
...
def parse_authorization_code_response(uri, state=None):
    ...
    **state**
    ...
    if not 'code' in params:
        ...
        print('Missing code is not allowed')
        return None

    if state and params.get('state', None) != state:
        #print('Mismatching state is not allowed')
        #return None
...
def my_parse_authorization_code_response(uri, state=None, params = None):
    #if not is_secure_transport(uri.get('uri')):
    if not is_secure_transport(uri):
        raise InsecureTransportError()
    ...
    # params = SymbolicDict(urlparse.parse_qsl(query))
...
def parse_implicit_response(uri, state=None, scope=None):
    ...
    **state**
    ...
    tmp = dict()
    if fragment != "":
        key_value_paris = fragment.split('&')
        for item in key_value_paris:
            #params[key] = value
            key,value = item.split('=',1)
            tmp[key] = value
    #params = dict(urlparse.parse_qsl(fragment, keep_blank_values=True))
    params = SymbolicDict({}, tmp)
    ...
    if state and params.get('state', None) != state:
        #print("Mismatching or missing state in params.")
        #return None
