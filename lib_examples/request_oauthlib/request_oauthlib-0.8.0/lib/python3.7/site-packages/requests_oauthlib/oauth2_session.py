...
class OAuth2Session(requests.Session):
    ...
    def fetch_token(self, token_url, code=None, authorization_response=None,
        ...
        if not is_secure_transport(token_url):
            ...
            #print('no secure transport is allowed')
            return None
    ...
    def my_fetch_token(self, token_url, code=None, authorization_response=None, params = None,
            body='', auth=None, username=None, password=None, method='POST',
            timeout=None, headers=None, verify=True, proxies=None, **kwargs):
        ...
        if not code and params:
            self._client.parse_request_uri_response(authorization_response, params = params, state=self._state)
            code = self._client.code
        elif not code and isinstance(self._client, WebApplicationClient):
            code = self._client.code
            if not code:
        ...
        #     log.debug('Invoking hook %s.', hook)
        #     r = hook(r)
        self._client.parse_request_body_response(r, scope=self.scope)
        # self._client.parse_request_body_response(r.text, scope=self.scope)
        self.token = self._client.token
        log.debug('Obtained token %s.', self.token)
        return self.token
